...
class YardService {
  ...
  async getDockedVehicles(warehouseId: string): Promise<DockingDto[]> {
    const response = await fetch(
      `${API_BASE_URL}/yard?warehouseId=${encodeURIComponent(warehouseId)}`
    );
  ...
  async updateGateStatus(
    ...
    console.log(
      `Updating gate status for waypoint ${waypointId}, dock ${dockId}, transport ${
        transportId || "none"
      }`
    );
   ...
   */
  async removeDockedVehicle(
    waypointId: string,
    vehicleId: string,
    transportId: string
  ): Promise<void> {
    console.log(
      `Removing docked vehicle ${vehicleId} from waypoint ${waypointId}`
    );

    const response = await fetch(`${API_BASE_URL}/yard/remove-vehicle`, {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ waypointId, vehicleId, transportId }),
    });
