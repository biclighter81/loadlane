...
public class DirectionsService
{
    ...
    public async Task<Route> GetOrCreateRouteWithWaypointsAsync(
        (double lng, double lat) start,
        (double lng, double lat) destination,
        IEnumerable<(double lng, double lat)> waypoints)
    {
        var waypointsList = waypoints.ToList();

        // Create cache key including all waypoints
        var waypointsStr = waypointsList.Count > 0
            ? string.Join("|", waypointsList.Select(w => $"{w.lng:F6},{w.lat:F6}"))
            : "none";
        var key = $"route:{_opts.Profile}:{start.lng:F6},{start.lat:F6}->{destination.lng:F6},{destination.lat:F6}:waypoints:{waypointsStr}";
        ...
        var coordsList = new List<string> { LonLat(start.lng, start.lat) };
        coordsList.AddRange(waypointsList.Select(w => LonLat(w.lng, w.lat)));
        coordsList.Add(LonLat(destination.lng, destination.lat));

        var coordsString = string.Join(";", coordsList);
        var url =
            $"https://api.mapbox.com/directions/v5/mapbox/{_opts.Profile}/{coordsString}" +
            $"?alternatives=false&geometries=geojson&overview=full&steps=false&access_token={Uri.EscapeDataString(_opts.AccessToken)}";
